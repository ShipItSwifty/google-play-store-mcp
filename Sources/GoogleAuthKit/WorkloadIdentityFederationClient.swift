...
/// recommendation), since the JSON-key flow depends on a downloadable key that policy blocks
/// from ever being created.
///
/// ## The three-step exchange
/// Mirrors what `google-github-actions/auth` does under the hood, done natively so a caller
...
public actor WorkloadIdentityFederationClient: Sendable {
    ...
    private let logger = Logger(label: "GoogleAuthKit.WorkloadIdentityFederation")
    ...
    private func fetchNewToken() async throws -> String {
        logger.info("Exchanging GitHub Actions OIDC token for Google credentials via \(provider)")
        ...
        logger.info("Impersonated \(serviceAccountEmail), token expires at \(expiresAt)")
        return accessToken
    }
    ...
    private func fetchGitHubActionsIDToken() async throws -> String {
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            ...
            throw GoogleAPIError.apiError(
                statusCode: status,
                body: "fetching the GitHub Actions OIDC token: \(String(data: data, encoding: .utf8) ?? "")"
            )
        }
        ...
        do {
            ...
            throw GoogleAPIError.decodingFailed(
                path: "github-actions-oidc-token", type: "TokenEnvelope", underlying: error)
        }
    }
    ...
    private func exchangeForFederatedToken(githubToken: String) async throws -> String {
        guard let url = URL(string: "https://sts.googleapis.com/v1/token") else {
            throw GoogleAPIError.invalidConfiguration(reason: "workload-identity-federation: invalid STS URL")
        }
        var request = URLRequest(url: url)
        ...
        do {
            ...
            throw GoogleAPIError.decodingFailed(
                path: "sts.googleapis.com/v1/token", type: "GoogleOAuth2TokenResponse", underlying: error)
        }
    }
    ...
    private func impersonateServiceAccount(federatedToken: String) async throws -> (String, Date) {
        ...
        guard let url = URL(string: endpoint) else {
            throw GoogleAPIError.invalidConfiguration(reason: "workload-identity-federation: invalid IAM Credentials URL")
        }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        ...
        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            ...
            throw GoogleAPIError.apiError(
                statusCode: status,
                body: """
                    \(body) — check that this GitHub repository is bound to \
                    roles/iam.workloadIdentityUser on \(serviceAccountEmail)
                    """
            )
        }

        let decoded: GenerateAccessTokenResponse
        do {
            decoded = try JSONDecoder().decode(GenerateAccessTokenResponse.self, from: data)
        } catch {
            throw GoogleAPIError.decodingFailed(
                path: "iamcredentials.googleapis.com:generateAccessToken",
